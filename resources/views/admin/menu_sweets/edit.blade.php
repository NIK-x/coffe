        ...
        <form action="{{ route('admin.menu-sweets.update', $menuSweet) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('name_sweet') is-invalid @enderror" 
                       id="name_sweet" name="name_sweet" value="{{ old('name_sweet', $menuSweet->name_sweet) }}" required>
                @error('name_sweet')
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('description_sweet') is-invalid @enderror" 
                          id="description_sweet" name="description_sweet" rows="3" required>{{ old('description_sweet', $menuSweet->description_sweet) }}</textarea>
            ...
            <div class="mb-3">
                ...
                <input type="number" step="0.01" class="form-control @error('price_sweet') is-invalid @enderror" 
                       id="price_sweet" name="price_sweet" value="{{ old('price_sweet', $menuSweet->price_sweet) }}" required>
                @error('price_sweet')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            <div class="mb-3">
                ...
                <select class="form-control @error('coffeeshop_sweet_bd') is-invalid @enderror" 
                    ...
                    @foreach($coffeeShops as $shop)
                        <option value="{{ $shop->id_coffee_shop }}" {{ old('coffeeshop_sweet_bd', $menuSweet->coffeeshop_sweet_bd) == $shop->id_coffee_shop ? 'selected' : '' }}>
